...
const penilaian_pembimbinglapangan = ref(null);
...
const getData = async () => {
    try {
        const response = await Api.get(`pembimbinglapangan/datasiswa/profile/${id}/berinilai`);
        dataAsli.value = response.data;
        data.value = response.data;
        penilaian_guru.value = data.value.penilaian_guru;
        penilaian_pembimbinglapangan.value = data.value.penilaian_pembimbinglapangan;
...
const formPenilaianPembimbinglapangan = ref(false);
...
const dataFormPenilaianPembimbinglapangan = ref([]);
const onSubmitPenilaianPembimbinglapangan = async (values) => {
    console.log(dataFormPenilaianPembimbinglapangan.value);
    let dataStore = {
        nilai: dataFormPenilaianPembimbinglapangan.value.nilai,
        penilaian_pembimbinglapangan_id: formPenilaianPembimbinglapangan.value.id,
    };
    // console.log(dataForm);
    try {
        const response = await Api.post(`pembimbinglapangan/datasiswa/profile/${id}/berinilai`, dataStore);
        ...
        // Toast.warning("Info", "Menu belum tersedia")
        getData();
        formPenilaianPembimbinglapangan.value = false;
        return true;
    } catch (error) {
        console.error(error);
    }
...
<template>
    ...
    <div class="md:py-1 px-1 lg:flex flex-wrap gap-1" v-if="siswa">
    ...
    <!-- nilai master guru -->
    <div v-if="penilaian_pembimbinglapangan">
        <div class="lg:flex w-full">
            <div class="w-full lg:w-2/3">
                <vue-good-table :line-numbers="true" :columns="columns" :rows="penilaian_pembimbinglapangan"
                    :search-options="{
                        enabled: true,
                    }" :pagination-options="{
    enabled: true,
    perPageDropdown: [10, 20, 50],
                    ...
                    <template #table-row="props">
                        <span v-if="props.column.field == 'actions'">
                            <div class="text-sm font-medium text-center flex justify-center space-x-1">
                                ...
                                <label class="flex">
                                    <button
                                        @click="(formPenilaianPembimbinglapangan = { id: props.row.id, penilaian_nama: props.row.penilaian_nama })"
            ...
            <div class="w-full lg:w-1/3 px-2">
                <div>
                    ...
                    <div v-if="formPenilaianPembimbinglapangan">
                        <Form v-slot="{ errors }" @submit="onSubmitPenilaianPembimbinglapangan">
                            <div class="py-2 lg:py-4 px-4">
                                <div class="space-y-4">
                                    <div class="flex flex-col">
                                        <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Nilai
                                            {{ formPenilaianPembimbinglapangan.penilaian_nama }}</label>
                                        <Field v-model="dataFormPenilaianPembimbinglapangan.nilai"
                            ...
                            <div class="w-full flex justify-end py-10 px-10 gap-4">
                                ...
                                <span class="btn btn-danger"
                                    @click="(formPenilaianPembimbinglapangan = false)">Batal</span>
                            </div>
                        </Form>
                    </div>
                </div>
